...
public class SendMail {
	...
	// 購入確定後のメール送信をするコンストラク
	// 引数 ArrayList<Order> orderList、int total(合計金額)
	public SendMail(ArrayList<Order> orderList , int total) {
		try {
			...
			// 送信元メールアドレスと送信者名を指定
			mimeMessage.setFrom(new InternetAddress("test.sender@kanda-it-school-system.com", "ユニフォーム受注管理システム", "iso-2022-jp"));

			// 送信先メールアドレスを指定（ご自分のメールアドレスに変更）
			mimeMessage.setRecipients(Message.RecipientType.TO, "r-matsuura@zyyx,jp");

			// メールのタイトルを指定
			mimeMessage.setSubject("購入内容確認", "iso-2022-jp");
	...
	// 入金状況、発送状況の変更メール
	// 引数 name(名前) payment
	public SendMail(String name, String payment, String send) {
		try {
			Properties props = System.getProperties();
